...
--Determine Day of the week with most User engagment for Each application

SELECT A.title, TO_CHAR(R.posted_at, 'Day') as day_of_week, COUNT(R.posted_at) as active_users
FROM apps A
	JOIN reviews R
	ON A.appid = R.appid
GROUP BY A.title, day_of_week
ORDER BY A.title, day_of_week;
